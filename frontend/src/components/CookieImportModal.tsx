...
export default function CookieImportModal({ proxies, busy, onClose, onSave }: { proxies: ProxyRecord[]; busy: boolean; onClose: () => void; onSave: (input: CookieImportInput) => void }) {
  ...
  return <div className="modal-overlay" onClick={onClose}><form className="modal" onClick={(e) => e.stopPropagation()} onSubmit={(e) => { e.preventDefault(); onSave({ cookie, name: name || undefined, proxy_id: proxyId || null }) }}>
