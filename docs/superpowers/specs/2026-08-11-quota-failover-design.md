...
5. 关于**流式**：实现上无需为成功的流式响应增加任何缓冲。`should_forward_stream`
   门（`proxy.rs:186`）已保证非成功响应（含 402/429）在回给客户端**之前整包缓冲**，
   额度错误只可能出现在非成功响应里，因此直接在该已缓冲的 body 上判定：
   - 额度错误 → 换号重试；
   - 否则 → 按现有逻辑把已缓冲的错误 body 原样回给客户端。
   - 成功流式响应保持**即时透传**，首帧延迟无任何增加。
